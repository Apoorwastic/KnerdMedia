    ...
    <Modal open={true} onClose={onClose} title={editing ? 'Edit task' : 'Details'} size="lg">
      {editing ? (
        <div className="space-y-4">
          ...
          {/* Assignees */}
          <div>
            <label className={labelClass + ' mb-2'}>{form.type === 'MEETING' ? 'Attendees' : 'Assigned to'}</label>
          ...
          <div className="flex gap-2 pt-1">
            <button onClick={() => updateTask.mutate()} disabled={updateTask.isPending} className="flex-1 bg-[#00d4c8] text-[#0a1628] rounded-xl py-2.5 text-sm font-bold hover:bg-[#00b8ac] disabled:opacity-50 transition-colors">Save changes</button>
        ...
        <div className="space-y-5">
          ...
          {task.description && (
            <div>
              <p className="text-xs font-semibold text-[#4a6278] uppercase tracking-wide mb-1">Description</p>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <p className="text-xs text-[#4a6278] mb-1 flex items-center gap-1"><CalendarDays size={11} /> Date</p>
              <p className="font-medium text-white">{task.dueDate ? format(new Date(task.dueDate), 'MMM d, yyyy') : '—'}</p>
            </div>
            {task.time && (
              <div>
          ...
          <div className="border-t border-[#1e3a5f] pt-4">
            ...
            <div className="flex gap-2">
              <input className="flex-1 border border-[#1e3a5f] bg-[#162032] text-white placeholder:text-[#4a6278] rounded-xl px-3 py-2 text-sm focus:border-[#00d4c8] focus:outline-none transition-colors" placeholder="Add a comment..." value={comment} onChange={e => setComment(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && comment.trim()) addComment.mutate(); }} />
              <button onClick={() => addComment.mutate()} disabled={!comment.trim()} className="p-2 bg-[#00d4c8] text-[#0a1628] rounded-xl disabled:opacity-40 hover:bg-[#00b8ac] transition-colors">
                <Send size={14} />
              </button>
            </div>
          </div>
